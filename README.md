...
You'll see something this come up. It has already select {Tab} as the delimeter for you! Make sure the character set is Unicode (UTF-8), and make sure Text delimeter is blank. (It might be a quotation mark). Make sure "quote all text cells" is NOT checked.

![](/readme_assets/libre_tsv_2.png)


All that said, a specific CSV format is also available.
...
CSV files can be generated by Excel and LibreOffice, but Excel in particular is not recommended as it likes to only selectively quote fields (ie, it only puts quotation marks around fields if they have a comma in them). It seems to do okay if you use TSV file format, however. LibreOffice gives you full control over the export of TSV/CSV files, letting you set field and text delimters and quoting rules.
